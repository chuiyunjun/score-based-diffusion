# derived from https://github.com/hvy/chainer-inception-score/blob/master/inception_score.py
...
def FID(m0, c0, m1, c1):
    ret = 0
    ret += jnp.sum((m0 - m1) ** 2)
    ret += jnp.trace(c0 + c1 - 2.0 * scipy.linalg.sqrtm(jnp.dot(c0, c1)))
    return jnp.real(ret)

def get_mean_cov(ims, batch_size=100):
    model = Inception()
